...
def validate(model, dataloader, device, normalize_method, statistic,
    ...
    for i in range(9):
        score_norm = evaluate(target_norm[:,0,i], forecast_norm[:,0,i])
        print(f'Evalute for column: {i}')
        print(f'Value: MAPE {score_norm[0]:7.9%}; MAE {score_norm[1]:7.9f}; RMSE {score_norm[2]:7.9f}.')
